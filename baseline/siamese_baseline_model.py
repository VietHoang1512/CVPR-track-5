...
class SiameseBaselineModel(torch.nn.Module):
    def __init__(self, model_cfg):
        ...
        self.resnet50 = resnet50(pretrained=False, num_classes=model_cfg.OUTPUT_SIZE)
        state_dict = torch.load(self.model_cfg.RESNET_CHECKPOINT, map_location=lambda storage, loc: storage.cpu())
        del state_dict["fc.weight"]
        del state_dict["fc.bias"]
        self.resnet50.load_state_dict(state_dict, strict=False)
        self.bert_tokenizer = BertTokenizer.from_pretrained(self.model_cfg.BERT_CHECKPOINT)
        self.bert_model = BertModel.from_pretrained(self.model_cfg.BERT_CHECKPOINT)
        self.lang_fc = torch.nn.Linear(768, model_cfg.OUTPUT_SIZE)

    def forward(self, track):
        nl = track["nl"]
        tokens = self.bert_tokenizer.batch_encode_plus(nl, padding="longest", return_tensors="pt")
        outputs = self.bert_model(tokens["input_ids"].cuda(), attention_mask=tokens["attention_mask"].cuda())
    ...
    def compute_lang_embed(self, nls, rank):
        with torch.no_grad():
            tokens = self.bert_tokenizer.batch_encode_plus(nls, padding="longest", return_tensors="pt")
            outputs = self.bert_model(
                tokens["input_ids"].cuda(rank), attention_mask=tokens["attention_mask"].cuda(rank)
            )
            lang_embeds = torch.mean(outputs.last_hidden_state, dim=1)
            lang_embeds = self.lang_fc(lang_embeds)
        return lang_embeds
