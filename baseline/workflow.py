...
flags.DEFINE_integer(
    "local_machine", 0, "Master node is 0, worker nodes starts from 1." "Max should be num_machines - 1."
)

flags.DEFINE_integer("num_gpus", 2, "Number of GPUs per machines.")
flags.DEFINE_string("config_file", "baseline/default.yaml", "Default Configuration File.")
...
def train_model_on_dataset(rank, train_cfg):
    ...
    dist.init_process_group(
        backend="nccl", rank=dist_rank, world_size=train_cfg.WORLD_SIZE, init_method=train_cfg.INIT_METHOD
    )
    dataset = CityFlowNLDataset(train_cfg.DATA)
    model = SiameseBaselineModel(train_cfg.MODEL).cuda()
    model = DistributedDataParallel(
        model, device_ids=[rank], output_device=rank, broadcast_buffers=train_cfg.WORLD_SIZE > 1
    )
    train_sampler = DistributedSampler(dataset)
    dataloader = DataLoader(
        dataset, batch_size=train_cfg.TRAIN.BATCH_SIZE, num_workers=train_cfg.TRAIN.NUM_WORKERS, sampler=train_sampler
    )
    optimizer = torch.optim.SGD(
        params=model.parameters(), lr=train_cfg.TRAIN.LR.BASE_LR, momentum=train_cfg.TRAIN.LR.MOMENTUM
    )
    lr_scheduler = StepLR(optimizer, step_size=train_cfg.TRAIN.LR.STEP_SIZE, gamma=train_cfg.TRAIN.LR.WEIGHT_DECAY)
    if rank == 0:
        if not os.path.exists(os.path.join(train_cfg.LOG_DIR, train_cfg.EXPR_NAME)):
            os.makedirs(os.path.join(train_cfg.LOG_DIR, train_cfg.EXPR_NAME, "summary"))
            os.makedirs(os.path.join(train_cfg.LOG_DIR, train_cfg.EXPR_NAME, "checkpoints"))
    ...
    for epoch in range(train_cfg.TRAIN.START_EPOCH, train_cfg.TRAIN.EPOCH):
        ...
        for data in dataloader:
            ...
            if global_step % train_cfg.TRAIN.PRINT_FREQ == 0:
                if rank == 0:
                    _logger.info("EPOCH\t%d; STEP\t%d; LOSS\t%.4f" % (epoch, global_step, loss.data.item()))
            global_step += 1
        if rank == 0:
            checkpoint_file = os.path.join(
                train_cfg.LOG_DIR, train_cfg.EXPR_NAME, "checkpoints", "CKPT-E%d-S%d.pth" % (epoch, global_step)
            )
            torch.save(
                {
                    "epoch": epoch,
...
def eval_model_on_dataset(rank, eval_cfg, queries):
    ...
    if rank == 0:
        if not os.path.exists(os.path.join(eval_cfg.LOG_DIR, eval_cfg.EXPR_NAME)):
            os.makedirs(os.path.join(eval_cfg.LOG_DIR, eval_cfg.EXPR_NAME, "logs"))
        with open(os.path.join(eval_cfg.LOG_DIR, eval_cfg.EXPR_NAME, "config.yaml"), "w") as f:
            f.write(eval_cfg.dump())
    dataset = CityFlowNLInferenceDataset(eval_cfg.DATA)
    model = SiameseBaselineModel(eval_cfg.MODEL)
    ckpt = torch.load(eval_cfg.EVAL.RESTORE_FROM, map_location=lambda storage, loc: storage.cpu())
    ...
    dataloader = DataLoader(dataset, batch_size=eval_cfg.EVAL.BATCH_SIZE, num_workers=eval_cfg.EVAL.NUM_WORKERS)
    ...
    for idx, query_id in enumerate(queries):
        ...
        with open(os.path.join(eval_cfg.LOG_DIR, eval_cfg.EXPR_NAME, "logs", "%s.log" % query_id), "w") as f:
...
if __name__ == "__main__":
    ...
    if cfg.TYPE == "TRAIN":
        mp.spawn(train_model_on_dataset, args=(cfg,), nprocs=cfg.NUM_GPU_PER_MACHINE, join=True)
    elif cfg.TYPE == "EVAL":
        ...
        mp.spawn(eval_model_on_dataset, args=(cfg, queries), nprocs=cfg.NUM_GPU_PER_MACHINE, join=True)
