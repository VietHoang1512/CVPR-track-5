...
class CVPRModel(nn.Module):
    def __init__(self, image_model, bert_model, n_hiddens, out_features, device):
        super().__init__()

        self.background_image_model, background_preprocess = clip.load(image_model, device=device, jit=False)
        self.cropped_image_model, cropped_preprocess = clip.load(image_model, device=device, jit=False)
        n_features = (
...
output_dir = os.path.join(CFG.OUTPUT_DIR, "triplet_clip")
